...
class RtreenodeMinheap():
    ...
    def insert(self, vertex):
        # adjust the priority queue structure(min heap)
        self.heap.append(vertex)
        i = len(self.heap) - 1
        while i > 1:
            parent = i // 2
            if self.compare_dist(self.heap[parent], vertex):
                # if the current vertex is closer than the heap[parent], then need to adjust
    ...
    def delete(self):
        ...
        if return_value.is_leaf:
            if isinstance(return_value, rtree.RTreeEntry):
                self.min_heapify(1)
                print(str(return_value.rect)+",  Distance:"+str(self.dist(return_value)))
                # print(return_value)
                return return_value
    ...
    def dist(self, e):
        """
        return the minDist of query_point and other MBRs.
        """
